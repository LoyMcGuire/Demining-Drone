...
byte CTRL1              = 0B00100000;
byte CTRL5              = 0B00100100;
byte CTRL7              = 0B00100110;
...
byte Address  = 0B00111010;  //address of accelerometer/magnetometer with SAO connected to Vdd

int magnitudeT=-0;
int magnitudeK=0;
float latitude, longitude;            //GPS variables

TinyGPSPlus gps;                      //instanciating the GPS chip

bool magFound = false;
  
byte X_L_M, X_H_M, Y_L_M, Y_H_M, Z_L_M, Z_H_M;
byte X_L_A, X_H_A, Y_L_A, Y_H_A, Z_L_A, Z_H_A;
int X_AXIS_M, Y_AXIS_M, Z_AXIS_M;
int X_AXIS_A, Y_AXIS_A, Z_AXIS_A;
...
void setup(){
  ...
  DDRE |= B00110000;                                                                    //Configure digital port 2 and 3 as outputs.
  DDRB |= B11110000;                                                                    //Configure digital port 10, 11, 12, and 13 as output.
  ...
  Serial3.begin(57600);                 //starting the GPS serial 
  
  //activate accelerometer/magnetometer  
  WriteRegister(CTRL1,0B00110111);
  WriteRegister(CTRL5,0B00001000);
  WriteRegister(CTRL7,0B00000000);
...
void loop(){
  ...
  X_L_M = ReadRegister(OUT_X_L_M);
  X_H_M = ReadRegister(OUT_X_H_M);
  Y_L_M = ReadRegister(OUT_Y_L_M);
  Y_H_M = ReadRegister(OUT_Y_H_M);
  Z_L_M = ReadRegister(OUT_Z_L_M);
  Z_H_M = ReadRegister(OUT_Z_H_M);
  
  X_L_A = ReadRegister(OUT_X_L_A);
  X_H_A = ReadRegister(OUT_X_H_A);
  Y_L_A = ReadRegister(OUT_Y_L_A);
  Y_H_A = ReadRegister(OUT_Y_H_A);
  Z_L_A = ReadRegister(OUT_Z_L_A);
  Z_H_A = ReadRegister(OUT_Z_H_A);
  
  X_AXIS_M = X_H_M <<8 | X_L_M;
  Y_AXIS_M = Y_H_M <<8 | Y_L_M;
  Z_AXIS_M = Z_H_M <<8 | Z_L_M;
  
  X_AXIS_A = X_H_A <<8 | X_L_A;
  Y_AXIS_A = Y_H_A <<8 | Y_L_A;
  Z_AXIS_A = Z_H_A <<8 | Z_L_A;
  
  if (magnitudeT == 0) {                  // Initialize first value if beginning program
    magnitudeT = Z_AXIS_M;
  }
  magnitudeK = magnitudeT;
  magnitudeT = Z_AXIS_M;

  difference = abs(magnitudeK - magnitudeT);
  
  //-------------------------------------------------GPS----------------------------------------------------
  while (Serial3.available() > 0){        //checks to see if the serial pins are available 
    if (gps.encode(Serial3.read())){      //endodes the data to the serial pins
      if (gps.location.isValid()){        //checks to see if the GPS has a fix and the data is valid
        latitude = gps.location.lat();    //updates the latitude with newer data
        longitude = gps.location.lng();   //updates the longitude with newer data
      }
    }
  }
  
//----------------------------------------------------------------------------------------------------------
  if (abs(magnitudeK - magnitudeT) > 150) {
    magnitudeT = 0;
    magFound = true;
    digitalWrite(3, HIGH);
  } else {
